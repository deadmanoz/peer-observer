...
///
/// Retry attempts are increased to handle slower hardware where bitcoind
/// warmup can exceed corepc_node's default wallet-ready timeout. The binary is resolved
/// from the `BITCOIND_EXE` environment variable (via `corepc_node::exe_path`);
/// if unset, a pre-built binary is downloaded automatically.
